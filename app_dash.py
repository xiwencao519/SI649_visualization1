# app_dash.py (full code with flat + 3D support, unmodified)
...
anomaly_data = {}
print("Preprocessing yearly mean anomalies...")
for year in years:
    anomaly = ds.sel(time=slice(f"{year}-01", f"{year}-12"))["tempanomaly"].mean(dim="time")
    anomaly_data[year] = anomaly.fillna(0)
print("✅ Done!")
...
def plot_flat_map_image(year, region):
    ...
    if region == 'north_america':
        ...
        ax.set_extent([50, 150, 5, 60], crs=ccrs.PlateCarree())
    elif region == 'oceania':
...
def plot_globe(year, region):
    ...
    for geom in world.geometry:
        ...
        for poly in polygons:
            ...
            coastlines.append(go.Scatter3d(x=x_line, y=y_line, z=z_line, mode='lines', line=dict(color='black', width=1), hoverinfo='skip', showlegend=False))
...
app.layout = html.Div([
    html.H2("GISTEMP Global Temperature Anomalies Viewer"),
    ...
    html.Label("Map Type:"),
    dcc.RadioItems(id='map-type', options=[
        {"label": "Flat Map", "value": "flat"},
        {"label": "3D Globe", "value": "3d"}
    ], value='flat', inline=True),
